      ...
      <div className="grid lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-4 lg:sticky lg:top-28 space-y-6">
          <div className="relative group">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within:text-[#e8a020] transition-colors" size={16} />
            <input 
              type="text" 
          ...
          {selectedItem ? (
            <div className="space-y-8 animate-fade-in">
              ...
              <div className="card-premium p-10 md:p-12 relative">
                ...
                <h2 className="text-3xl font-black text-white mb-8 tracking-tight">{selectedItem.title}</h2>
                      ...
                      <thead>
                        <tr className="bg-white/5">
                          {selectedItem.headers.map(h => (
                            <th key={h} className="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] border-b border-white/5">{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody className="text-slate-300">
                        {selectedItem.rows.map((row, i) => (
                          <tr key={i} className="hover:bg-white/5 transition-colors">
                            {row.map((cell, j) => (
                              <td key={j} className="px-6 py-4 border-b border-white/[0.02] text-sm font-bold">{cell}</td>
                            ))}
                          </tr>
                        ))}
                      </tbody>
